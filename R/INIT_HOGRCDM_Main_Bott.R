INIT_HOGRCDM_Main_Bott<-function(ModelSetList,SizeList,Res,Q_B){
  N<-SizeList$N
  J<-SizeList$J
  K<-SizeList$K
  D<-SizeList$D
  ...
  if(ModelSetList$ModelDistri_Bottom=="Lognormal"){
    ...
    if( (ModelSetList$ModelFrame_Bottom=="Confirmatory") ) {
      ...
      ########################Initilization for bottom layer
      INI_B<-INI_CC_LLM(DATA=Res, N,J,K,eplison_Layer0=1e-04)
  ...
  if((ModelSetList$ModelStruc_Bottom=="All_effect")){
    ...
    if(ModelSetList$ModelFrame_Bottom=="Exploratory"){
      Slope_B_INI_Aug<- matrix(0, J, L-1)
      Slope_B_INI_Aug[,1:K]<- Slope_B_INI
      Slope_B_INI<-  Slope_B_INI_Aug
      
    }else if(ModelSetList$ModelFrame_Bottom=="Confirmatory"){
