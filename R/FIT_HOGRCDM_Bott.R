FIT_HOGRCDM_Bott<-function(ModelSetList,SizeList,Res,Q_H,Com_par=NULL,SettingList){
  ...
  if(ModelSetList$ModelDistri_Bottom=="Lognormal"){
    ...
    if( (ModelSetList$ModelFrame_Bottom=="Confirmatory") ){
      
      FIT<-FIT_C_Lognormal(Res, SizeList, InitAll, SettingList,
                            ModelSetList, Q_H,Q_B)
      
    }else if((ModelSetList$ModelFrame_Bottom=="Exploratory")){
      ...
      for(lu in 1:LL){
        ...
        tryCatch({
          FIT<- FIT_E_Lognormal(Res, SizeList, InitAll, SettingList,ModelSetList, Q_H,RegurlPara,Com_par)
    ...
    if( (ModelSetList$ModelFrame_Bottom=="Confirmatory") ){
      ...
      for(lu in 1:LL){
        ...
        RegurlPara<-rep(RegurlParaVec[lu],J);
        tryCatch({
          FIT<- FIT_E_LLM(Res, SizeList, InitAll, SettingList,ModelSetList, Q_H,RegurlPara,Com_par)
          Par_size=sum(as.integer( FIT$Slope_B>RegurlParaVec[lu]/5))
          H_Num<-Par_size+sum(Q_Higher)+K;#The number of non-zero parameters
          EBIC[lu]<- -2*FIT$LIKELI+ H_Num*log(N)
          #+2*1*log(nchoosek_prac(sum(Q_Higher)+K+(K+1)*J,sum(Q_Higher)+K+Par_size))
    ...
    if( (ModelSetList$ModelFrame_Bottom=="Confirmatory") ){
      ...
      FIT<-FIT_CE_Poisson(Res, SizeList, InitAll, SettingList,
                          ModelSetList, Q_H,Q_B)
      ...
      for(lu in 1:LL){
        ...
        RegurlPara<-rep(RegurlParaVec[lu],J);
        tryCatch({
          FIT<- FIT_E_Poisson(Res, SizeList, InitAll, SettingList,ModelSetList, Q_H,RegurlPara,Com_par)
          Par_size=sum(as.integer( FIT$Slope_B>RegurlParaVec[lu]/5))
          H_Num<-Par_size+sum(Q_Higher)+K;#The number of non-zero parameters
          EBIC[lu]<- -2*FIT$LIKELI+ H_Num*log(N)
          #+2*1*log(nchoosek_prac(sum(Q_Higher)+K+(K+1)*J,sum(Q_Higher)+K+Par_size))
    ...
    if( (ModelSetList$ModelFrame_Bottom=="Confirmatory") ){
      ...
      for(lu in 1:LL){
        ...
        RegurlPara<-rep(RegurlParaVec[lu],J);
        tryCatch({
          FIT<- FIT_E_Gamma(Res, SizeList, InitAll, SettingList,ModelSetList, Q_H,RegurlPara,Com_par)
          Par_size=sum(as.integer( FIT$Slope_B>RegurlParaVec[lu]/5))
          H_Num<-Par_size+sum(Q_Higher)+K;#The number of non-zero parameters
          EBIC[lu]<- -2*FIT$LIKELI+ H_Num*log(N)
          #+2*1*log(nchoosek_prac(sum(Q_Higher)+K+(K+1)*J,sum(Q_Higher)+K+Par_size))
