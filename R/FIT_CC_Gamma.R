...
FIT_CC_Gamma<-function(Res, SizeList, InitAll, SettingList,
  ...
  log_likelihood <- function(j,alpha, ABP,Y) {
    # if(alpha <= 0) return(-Inf)  # avoid non-positive alpha
    sum(-lgamma(alpha)*nrow(Y) + alpha *(ABP[j]) +(alpha-1)*sum(log(Y[,j])))
  }
  ...
  while((passing<Passing & i<maxIter)){
    ...
    Beta<-sapply(1:J, OptBott_Confir, x=Data_x,
                 Res=Data_Res,Q_mat=Q_Aug,weights=WEI,Family="Gamma", Link="inverse",
                 simplify = T)
    
  
    Interc_B <- Beta[1,]*Shape
    Slope_B <- t(Beta[-1,])*Shape
  ...
  return(list(Slope_H=Slope_H,Interc_H=Interc_H,Sigma_theta=Sigma_theta,
              Interc_B=Interc_B,Slope_B=Slope_B,Shape=Shape,
